...
/*   Updated: 2021/11/08 20:15:22 by rotrojan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	open_semaphores(t_table *table)
{
	if (initialize_semaphore(SEM_FORKS, &table->sem_forks, table->nb_philo)
		== False)
		return (False);
	if (initialize_semaphore(SEM_WRITE, &table->sem_write, 1)
		== False)
		return (False);
	if (initialize_semaphore(SEM_STOP, &table->sem_stop, 1)
		== False)
		return (False);
	if (initialize_semaphore(SEM_SYNC_START, &table->sem_sync_start, 0)
		== False)
		return (False);
	if (initialize_semaphore(SEM_EAT, &table->sem_eat, 0)
		== False)
		return (False);
